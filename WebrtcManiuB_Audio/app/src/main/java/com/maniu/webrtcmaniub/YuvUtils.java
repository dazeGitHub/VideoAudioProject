...
//        nv21   0----nv21.size
    ...
    public  static  void writeBytes(byte[] array) {
        ...
        try {
            ...
            writer = new FileOutputStream(Environment.getExternalStorageDirectory() + "/codec.pcm", true);
            writer.write(array);
            writer.write('\n');
    ...
    public  static String writeContent(byte[] array) {
        ...
        try {
            // 打开一个写文件器，构造函数中的第二个参数true表示以追加形式写文件
            writer = new FileWriter(Environment.getExternalStorageDirectory() + "/codecPcm.txt", true);
