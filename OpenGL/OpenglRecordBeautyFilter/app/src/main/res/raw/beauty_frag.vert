...
//每个片元程序都会执行当前的 beauty_frag.vert 片元程序,每个片元程序都会给不同的坐标 aCoord
varying vec2 aCoord;
...
//  如果 width = 500, height = 500, 那么 singleStepOffset = (1/500, 1/500) * 500 (这里要乘500) = (1, 1)
    vec2 singleStepOffset = vec2(1.0 / float(width), 1.0 / float(height)); //步长  一个宽度是一份

    //= [500,500] + singleStepOffset * vec2(-10.0, 0.0)
    //= [500,500] + (1, 1) * vec2(-10.0, 0.0)
    //= [490, 500]
...
//    一环
    ...
    vec4 blur = vec4(rgb * 1.0/21.0, currentColor.a); //因为还包含中间点的颜色, 所以不是除以 20 而是 21
...
//  这就是高反差图片
    ...
    //    PS 中蓝色通道的轮廓更清晰  作为    参考  叠加
    //选取  原图的蓝颜色 和 高斯模糊的蓝颜色 中最小的那个
    float b = min(currentColor.b, blur.b);

    //    对蓝色通道进行线性叠加  (b - 0.2) * 5.0
...
//  两个图     currentColor 是原图     blur.rgb 是高斯模糊图     currentIntensity
