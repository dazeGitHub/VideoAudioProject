...
//如果在其他线程回调给主线程, 那么必须实现 JNI_OnLoad() 方法得到 javaVM 指针
...
//        有数据    就会取出来  没有数据   阻塞
        ...
        if (ret >= 0) {
//          audioFrame->data;  --->数据源
//          outbuffer  --->目的地, 长度是 44100 * 2
//          nb_samples : 解码后的音频帧有多少个
            swr_convert(swrContext, &outbuffer, 44100 * 2,
...
//            转换个数
            ...
            LOGD("获取pcm数据 %d  ", size);
//          将 javaVM 挂靠到 jniEnv 里
            if (javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK) {
                continue;
            }
//          java数组的容器
            jbyteArray byteArrays = jniEnv->NewByteArray(size);
//          将 outbuffer 数据放到 byteArrays 里面
            jniEnv->SetByteArrayRegion(byteArrays, 0, size,reinterpret_cast<const jbyte *>(outbuffer));
//
//          回调  能 1  不能2  jni  的子线程
            jniEnv->CallVoidMethod(mInstance, playTrack, byteArrays, size);
...
//  调用 java 层的 createTrack(int sampleRateInHz, int nb_channals) 方法, 将 采样频率和通道数 传递给 java 层
    jmethodID createAudio = env->GetMethodID(david_player, "createTrack", "(II)V");
//  gc管不住  不需要   对象  你没有任何通知 c 弱引用  1  有  2没有
//  这样写后 jvm 释放 instance 会通知自己, 这时全局引用 (c 中没有回收机制, 也就没有弱引用)
...
//        打开解码器
            ...
            audioIndex = i;
            AVCodecParameters *parameters = avFormatContext->streams[i]->codecpar;
            LOGI("david", "parameters -> sample_rate = %d  parameters -> channels = %d", parameters -> sample_rate, parameters -> channels);
//          nv->CallVoidMethod(instance, createAudio, parameters -> sample_rate, parameters -> channels);
            AVCodec * dec = avcodec_find_decoder(parameters->codec_id);
...
//  这里开设的线程不归 gc 管, 在子线程无法获取主线程的任何信息
