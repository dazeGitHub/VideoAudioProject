...
//这个方法一定是 C 方法, 因为调用的是 Linux 中 c 的开线程的方法
void MNAudio::play() {
...
//             mnAudio->clock  永远大于   帧 携带时间    配置
            ...
            (*mnAudio->pcmBufferQueue)->Enqueue(mnAudio->pcmBufferQueue, mnAudio->buffer,
                                                buffersize);
        }
    }
...
//    创建引擎 engineObject 直接拿到对应的接口, 对象不能直接操作, 而是通过接口的方式操作
    SLresult result;
...
//  创建混音器
    ...
    result =(*engineEngine)->CreateOutputMix(engineEngine, &outputMixObject, 1, mids, mreq);//创建一个那么值就是 1
    if(result != SL_RESULT_SUCCESS) {
        return;
    }
    LOGE("-------->initOpenSLES  2");
...
//SLDataSource *pAudioSrc,  音频源  音频配置
    ...
    const SLboolean req[3] = {SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE}; //ids 对应的开关值就是 req
