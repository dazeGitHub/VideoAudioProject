...
// 1s
        int buffersize = mnAudio->getSoundTouchData();
//        int buffersize = mnAudio->resampleAudio(mnAudio->out_buffer);
        if(buffersize > 0)
        {
...
//            往喇叭 方整理后 波形 facc没有   整理波形
//          mnAudio-> sampleBuffer 是经过变形后的波形
            (*mnAudio->pcmBufferQueue)->Enqueue(mnAudio->pcmBufferQueue,
                                                (char *) mnAudio-> sampleBuffer,
                                                buffersize * 2 * 2);
//          buffersize  是一个通道的 采样个数, 2个通道  一个采样2个字节, 所以是 buffersize * 2 * 2
        }
    }
...
//设值 高音 低音
void MNAudio::setHigth(float higth) {
    if (soundTouch != NULL) {
        soundTouch->setPitch(higth);
    }
}
